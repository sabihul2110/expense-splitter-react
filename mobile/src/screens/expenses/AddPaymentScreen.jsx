  ...
  async function loadPendingSplits() {
    ...
    try {
      const { data } = await client.get(
        `/payments/pending-splits/${groupId}?debtor_id=${user.user_id}&creditor_id=${payeeId}`
      );
      setPendingSplits(data || []);
      // Pre-select all pending splits
      setSelectedIds(new Set((data || []).map(s => s.expense_id)));
      // Auto-fill amount
      const total = (data || []).reduce((sum, s) => sum + s.remaining, 0);
          ...
          <View style={s.card}>
            ...
            {pendingSplits.length > 0 && (
              <View style={s.splitSummaryRow}>
                <Text style={s.splitSummaryLabel}>Selected total</Text>
                <Text style={s.splitSummaryAmt}>₹{selectedTotal.toLocaleString('en-IN')}</Text>
              </View>
            )}
          </View>
        )}
            ...
            <View style={s.amountRow}>
              ...
              {amountOverride && (
                <TouchableOpacity
                  onPress={() => {
                    const total = pendingSplits
                      .filter(sp => selectedIds.has(sp.expense_id))
                      .reduce((sum, sp) => sum + sp.remaining, 0);
                    setAmount(total > 0 ? total.toFixed(2) : '');
                    setAmountOverride(false);
                  }}
                  style={s.resetAmtBtn}
