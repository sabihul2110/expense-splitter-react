 ...
 *
 * Sections:
 *  1. Hero card  — avatar, name, email, role badge, UPI
 *  2. Stats row  — total groups + net balance (fetched live)
 *  3. Edit Profile modal   — name, email, UPI
 *  4. Change Password modal — current / new / confirm
 *
 * Backend:
 *  GET  /auth/me              → already used by AuthContext
 *  PUT  /users/me             → update name/email/upi_id
 *  POST /auth/change-password → change password
 *
 * Design: follows existing CSS variables + class conventions.
 * No new global styles needed — uses .card, .btn-*, .form-*, .badge-*, .alert-*
 */
    ...
    <div style={{ position: "relative" }}>
      ...
      <button
        ...
        style={{
          ...
          color: "var(--text3)", display: "flex", padding: 3,
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box fade-up">
        ...
        <div className="modal-body">
          ...
          {success && <div className="alert alert-success" style={{ display: "flex", alignItems: "center", gap: 6 }}>{Icon.check(14)} Saved successfully!</div>}
              ...
              {form.confirm.length > 0 && (
                <div style={{ marginTop: 5, fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
                  {form.newPwd === form.confirm ? (
                    <><span style={{ color: "var(--success)", display: "flex" }}>{Icon.check(12)}</span><span style={{ color: "var(--success)" }}>Passwords match</span></>
                  ) : (
...
export default function Profile() {
  ...
  // ── Load groups + settlements for stats ──
  useEffect(() => {
    async function loadStats() {
      ...
      try {
        ...
        await Promise.all(groupList.map(async g => {
          try {
            const { data: settlements } = await api.get(`/settlements/${g.group_id}`);
  ...
  // ── After profile save, sync AuthContext so topbar name updates ──
  function handleProfileSave(freshUser) {
    // Merge updated fields into stored auth — keep access_token
    ...
    login(updated); // triggers AuthContext re-render everywhere
  }
      ...
      <AppShell title="Profile">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 24 }}>
          ...
          <div className="card card-p" style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              {statsLoading ? (
                ...
                <div style={{ fontSize: 22, fontWeight: 800, color: "var(--primary-h)", letterSpacing: "-0.02em" }}>
                  {groups.length}
                </div>
              )}
            </div>
          ...
          <div className="card card-p" style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{
              width: 40, height: 40, borderRadius: 10, flexShrink: 0,
              background: netBalance > 0 ? "rgba(16,185,129,0.12)" : netBalance < 0 ? "rgba(239,68,68,0.1)" : "var(--surface2)",
              color: netColor,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              {Icon.wallet(18)}
            ...
            <div>
              ...
              {statsLoading ? (
                ...
                <div style={{ fontSize: 22, fontWeight: 800, color: netColor, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums" }}>
                  {netLabel}
                </div>
              )}
          ...
          {[
            ...
            <div key={row.label} style={{
              ...
              <span style={{ fontSize: 14, color: row.value === "Not set" ? "var(--text3)" : "var(--text)", fontWeight: 500 }}>
                {row.value === "Not set" ? <em style={{ fontStyle: "normal", fontSize: 13 }}>Not set</em> : row.value}
              </span>
            </div>
          ))}
          {/* Edit trigger row */}
